...
```{r}
  opt <- list()
  opt$datasets <- c('PRISM','GDSC','CTRP')
  opt$sensitivity_measure <- 'auc'
  ...
  opt$prior.sensitive <- .6 # prior specification denoting cell is sensitive if greater than this value
...
```{r}
  ...
  if (opt$run_on_cluster) {
    ...
    base_dir <- '~/Downloads/Pharmacogenomics/PSets'
  }
```
...
Load prism.
```{r}
prism <- read.csv(file.path(base_dir,"prism/secondary-screen-dose-response-curve-parameters.csv"), stringsAsFactors=FALSE) %>% 
  as_tibble() %>%
...
prism <- prism %>%
  ...
  mutate(experiment = 'PRISM',
         measure = opt$sensitivity_measure) %>%
...
Load GDSC.
```{r}
gdsc <- read.csv(file.path(base_dir, 'prism/sanger-dose-response.csv'), stringsAsFactors = FALSE) %>% 
  as_tibble() %>%
...
Load CTRP.
```{r}
ctrp <- read.delim(file.path(base_dir,'prism/CTRPv2.0_2015_ctd2_ExpandedDataset/v20.data.curves_post_qc.txt'),stringsAsFactors = FALSE) %>%
  as_tibble() %>%
  select(experiment_id, area_under_curve, master_cpd_id, conc_pts_fit) %>%
  mutate(area_under_curve = area_under_curve/conc_pts_fit) %>%
  select(-conc_pts_fit)
cpds <- read.delim(file.path(base_dir,'prism/CTRPv2.0_2015_ctd2_ExpandedDataset/v20.meta.per_compound.txt'),stringsAsFactors = FALSE) %>%
  as_tibble()
experis <-  read.delim(file.path(base_dir,'prism/CTRPv2.0_2015_ctd2_ExpandedDataset/v20.meta.per_experiment.txt'),stringsAsFactors = FALSE) %>%
  as_tibble()
cellls <- read.delim(file.path(base_dir,'prism/CTRPv2.0_2015_ctd2_ExpandedDataset/v20.meta.per_cell_line.txt'),stringsAsFactors = FALSE) %>%
  as_tibble()
ctrp <- ctrp %>%
...
p <- sensitivity.tibble.list[['CTRP']] %>%
  ...
  bind_rows(sensitivity.tibble.list[['PRISM']]) %>% 
  group_by(drug) %>%
...
ggsave(p,file=file.path(opt$log_dir,meas,comp,paste('boxplot_raw_common_drugs.png',sep='')),width = 12, height = 8, units = "in",limitsize=FALSE)
saveRDS(p, file=file.path(opt$log_dir,meas,comp,paste('boxplot_raw_commons_drugs.RDS',sep='')))
```
