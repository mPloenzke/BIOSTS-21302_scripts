...
output_dir <- paste(comparison,'biomarkers',feature.types[1],sep='_')
...
if (include_individual_posteriors) {
  individual_posteriors <- readRDS(file.path(posteriors_dir,
                                             paste(comparison,'_fit_within_study',sep=''),
                                             'posterior.formatted.RDS'))
}
joint_posteriors <- readRDS(file.path(posteriors_dir,
                                      paste(comparison,'_fit_across_study',sep=''),
                                      'posterior.formatted.RDS')) 
...
all_effect_sizes <- all_effect_sizes %>%
  ...
  replace_na(list(isKnown = FALSE,
                  logFC=0,
                  P.val=1))
write_csv(all_effect_sizes, path=file.path(output_dir, 'all_effect_sizes.csv'), 
