...
```{r}
comparison <- "CCLE_GDSC1000_CTRPv2_FIMM_gCSI" # CCLE_GDSC
...
feature_types <- c("rna2") # mutation fusion cnv rna rna2 rnaseq
drug_name <- 'Crizotinib' # drug name
...
output_dir <- paste(paste(feature_types,collapse='_'),'pathway',drug_name,sep='_')
...
gene_drug_asociations <- gene_drug_asociations %>% filter(drug %in% drug_name)
```
    ...
    if (class(my_design) != 'try-error') {
      ...
      if (class(fit) != 'try-error') {
        ...
        if (class(fit) != 'try-error') {
          ...
          #          path=file.path(output_dir, paste('GO_',depVar,'_',study,'.csv',sep='')), 
          #          col_names = TRUE, append = FALSE)
          write_csv(kegg_pathways, 
                    path=file.path(output_dir, paste('KEGG_',depVar,'_',study,'.csv',sep='')), 
                    col_names = TRUE, append = FALSE)
        }
      }
    }
...
if (mean_common_cells) {
  ...
  ourData <- do.call(bind_rows, rez) %>% 
    ...
    na.omit() %>%
    inner_join(posterior %>% 
                 filter(Model_fit == 'joint',drug==drug_name) %>%
  ...
  ourData <- do.call(bind_rows, rez) %>% 
    ...
    right_join(do.call(bind_rows, rez) %>% 
                 ...
                 filter(drug==drug_name), by=c('cell','drug')) %>%
    na.omit() %>%
    inner_join(posterior %>% 
                 filter(Model_fit == 'joint',drug==drug_name) %>%
                 select(-Model_fit) %>%
  ...
  if (class(my_design) != 'try-error') {
    ...
    if (class(fit) != 'try-error') {
      ...
      if (class(fit) != 'try-error') {
        ...
        #write_csv(g_ontology,
        #          path=file.path(output_dir, paste('GO_',depVar,'.csv',sep='')), 
        #          col_names = TRUE, append = FALSE)
        write_csv(kegg_pathways, 
                  path=file.path(output_dir, paste('KEGG_',depVar,'.csv',sep='')), 
                  col_names = TRUE, append = FALSE)
      }
    }
  }
