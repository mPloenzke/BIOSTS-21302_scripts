...
Load prism.
```{r}
prism <- read.csv(file.path("~/Downloads/Pharmacogenomics/PSets/prism/secondary-screen-dose-response-curve-parameters.csv"), stringsAsFactors=FALSE) %>% 
  as_tibble() %>%
...
```{r}
intersection.fitted.list <- readRDS('~/Downloads/Pharmacogenomics/results/PRISM_GDSC_CTRP_fit2/intersection.fitted.list.RDS') # this one has zeros
#intersection.fitted.list <- readRDS('~/Downloads/Pharmacogenomics/results/PRISM_GDSC_CTRP_fit3/intersection.fitted.list.RDS') # this one has zeros
#intersection.fitted.list <- readRDS('~/Downloads/Pharmacogenomics/results/PRISM_GDSC_CTRP_fit_overlap/intersection.fitted.list.RDS')
```

Load posterior information for all cells/drugs/experiments.
```{r}
posterior.formatted <- readRDS('~/Downloads/Pharmacogenomics/results/PRISM_GDSC_CTRP_fit2/posterior.formatted.RDS') # this one has zeros
#posterior.formatted <- readRDS('~/Downloads/Pharmacogenomics/results/PRISM_GDSC_CTRP_fit3/posterior.formatted.RDS') # this one doesnt
posterior.overlap <- readRDS('~/Downloads/Pharmacogenomics/results/PRISM_GDSC_CTRP_fit_overlap/posterior.formatted.RDS') %>% separate(cell, into=c('cell','num'))
...
twoway_agreement %>%
  ...
  theme_bw()
ggsave('temp.png',width=12,height=7,units='in')
```
...
for (comp in names(intersection.fitted.list)) {
  ...
  pearson <- tibb %>% 
    ...
    summarize(realized_value = mean(realized_value, na.rm=TRUE), .groups='drop') %>% 
    spread(experiment, realized_value) %>% 
    na.omit() %>%
    rename(v1 = curr_studies[1], v2 = curr_studies[2]) %>%
    group_by(drug) %>%
  ...
  pearson_posterior <- tibb %>% 
    ...
    select(drug, cell, posterior_probability_sensitive, experiment) %>%
    group_by(drug, cell, experiment) %>% 
    summarize(posterior_probability_sensitive = mean(posterior_probability_sensitive, na.rm=TRUE), .groups='drop') %>% 
    spread(experiment, posterior_probability_sensitive) %>% 
    na.omit() %>%
    rename(v1 = curr_studies[1], v2 = curr_studies[2]) %>%
    group_by(drug) %>%
    ...
    ungroup()
  drug_types <- tibb %>%
    filter((drug_type == 'targeted' & posterior_probability_targeted >= .5) | (drug_type == 'broad' & posterior_probability_targeted < .5)) %>% 
...
twoway_agreement %>%
  ...
  facet_wrap(vars(comparison),ncol=5)
ggsave('temp.png',width=10,height=5,units='in')
```
