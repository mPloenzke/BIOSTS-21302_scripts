...
Options.
```{r}
results_dir <- '~/Downloads/Pharmacogenomics/results'
output_dir <- '~/Downloads/Pharmacogenomics/Writeups/figs'
```

# Twoway posteriors comparison

Load posterior information for just CCLE and GDSC studies.
```{r}
intersection.tibble.list <- readRDS(file.path(results_dir, 'CCLE_GDSC_fit_p60/intersection.fitted.list.RDS'))
posterior_drug_types <- readRDS(file.path(results_dir,'CCLE_GDSC_fit_p60/posterior_drug_types.RDS'))
posterior_drug_fits <- readRDS(file.path(results_dir,'CCLE_GDSC_fit_p60/posterior_drug_fits.RDS'))
posterior_drug_assignments <- readRDS(file.path(results_dir,'CCLE_GDSC_fit_p60/posterior_drug_assignments.RDS')) %>% filter(iteration == max(iteration))
posterior_cell_assignments <- readRDS(file.path(results_dir,'CCLE_GDSC_fit_p60/posterior_cell_assignments.RDS')) %>% filter(iteration == max(iteration))
```
...
```{r}
intersection.fitted.list <- readRDS(file.path(results_dir,'CCLE_GDSC_fit_p60/intersection.fitted.list.RDS'))
```
...
```{r}
intersection.fitted.list <- readRDS(file.path(results_dir,'PRISM_GDSC_CTRP_fit2/intersection.fitted.list.RDS')) # this one has zeros
p_zero <- tibble()
...
pp <- plot_dat %>%
  ...
  geom_text_repel(aes(label=Pathway), size=3.5) + 
  theme_bw()+ 
  theme(legend.position='none', 
...
ggsave(plot=pp, file.path(output_dir, 'pathways.pdf'),width=8,height=8,units='in')
```
