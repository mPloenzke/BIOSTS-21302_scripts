...
```{r}
intersection.fitted.list <- readRDS(file.path(results_dir,'CCLE_GDSC1000_gCSI_FIMM_CTRPv2_fit_within_study/intersection.fitted.list.RDS'))
...
```{r}
intersection.fitted.list <- readRDS(file.path(results_dir,'PRISM_GDSC_CTRP_zeros_included/intersection.fitted.list.RDS')) # this one has zeros
twoway_agreement <- tibble()
for (comp in names(intersection.fitted.list)) {
  tibb <- intersection.fitted.list[[comp]]
  curr_studies <- strsplit(comp,'_')[[1]]
...
```{r}
mplot <- readRDS(file.path(results_dir, "CCLE_GDSC_fit_within_study/posterior_drug_fits.RDS")) %>% 
  group_by(drug) %>% 
...
```{r}
vplot <- readRDS(file.path(results_dir, "CCLE_GDSC_fit_within_study/posterior_drug_fits.RDS")) %>% 
  group_by(drug) %>% 
  filter(n_distinct(experiment)>1) %>% 
  ungroup() %>% 
  filter(drug_type == 'broad') %>% 
