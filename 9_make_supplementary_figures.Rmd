...
output_dir <- '~/Downloads/Pharmacogenomics/Writeups/supplemental_figs/'
```
...
```{r}
intersection.fitted.list <- readRDS(file.path(results_dir,'CCLE_GDSC1000_gCSI_FIMM_CTRPv2_fit_p60/intersection.fitted.list.RDS'))
...
```{r}
mplot <- readRDS(file.path(results_dir, "CCLE_GDSC_fit_p60/posterior_drug_fits.RDS")) %>% 
  group_by(drug) %>% 
...
```{r}
vplot <- readRDS(file.path(results_dir, "CCLE_GDSC_fit_p60/posterior_drug_fits.RDS")) %>% 
  group_by(drug) %>% 
  filter(n_distinct(experiment)>1) %>% 
  ungroup() %>% 
  filter(drug_type == 'broad') %>% 
...
```{r}
intersection.fitted.list <- readRDS(file.path(results_dir, 'PRISM_GDSC_CTRP_zeros_excluded/intersection.fitted.list.RDS')) 
twoway_agreement <- tibble()
for (comp in names(intersection.fitted.list)) {
  tibb <- intersection.fitted.list[[comp]]
  curr_studies <- strsplit(comp,'_')[[1]]
