---
title: "Comparison between sensitivity measures"
...
include_individual_posteriors <- FALSE # for individual study comparisons, should the posterior fit via only that dataset be included (TRUE) or should it be compared to the joint fit posterior (FALSE)
individual_study_measures <- c('naive_v_cell_type','auc_recomputed_v_posterior_probability_sensitive')
joint_study_measures <- c('naive_v_cell_type','auc_recomputed_v_posterior_probability_sensitive')
base_log_dir <- paste(comparison,'sensitivity_comparison',feature.types[1],sep='_')
...
```{r}
if (!run_on_cluster) {
  joint_posteriors <- readRDS(file.path('~/Desktop/Concordance/results',
...
for (dataset in datasets) {
  ...
  for (feat in feature.types) {
    ...
    if (downsample_features) {
      ...
      top.variable <- names(var.sds[1:min(length(var.sds),5000)]) #5000
...
for (drug_for_biomarkers in all_drugs) {
  for (study in names(rez)) {
    ...
    for (depVar in individual_study_measures) {
      var1 <- strsplit(depVar, '_v_')[[1]][1]
      var2 <- strsplit(depVar, '_v_')[[1]][2]
      if (var1 %in% c('naive','cell_type') & (var2 %in% c('naive','cell_type'))) {
        ourData <- ourData %>% 
          mutate(new_outcome = case_when(naive=='sensitive' & cell_type=='resistant' ~ 0,
                                         naive=='resistant' & cell_type=='sensitive' ~ 1,
                                         TRUE ~ -1)) %>% 
          filter(new_outcome != -1)
      } else if (var1 %in% c('auc_recomputed','posterior_probability_sensitive') &
                 (var2 %in% c('auc_recomputed','posterior_probability_sensitive'))) {
        ourData <- ourData %>% 
          mutate(new_outcome = posterior_probability_sensitive-auc_recomputed)
      }
      if (length(unique(ourData$new_outcome))<2) {
        print(paste0('Only one outcome category defined for ', drug_for_biomarkers,'. Skipping calculation.'))
        next()
      }
      ...
      expr_data <- ourData %>% 
        select(-drug, -cell, -experiment, #-waterfall, 
               ...
               -posterior_probability_sensitive, -new_outcome) %>%
      ...
      # pull pheno data
      tmp <- ourData %>% 
        select(cell, new_outcome) %>% 
        rename(Sensitivity= new_outcome) %>%
