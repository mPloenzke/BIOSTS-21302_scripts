...
Options and packages. Must use the commit of PharmacoGx to ensure compatability.
```{r setup, include=FALSE}
rm(list=ls(all=TRUE))
...
First download the full data sets. Move them to the file path specified in the next chunk.
```{r,eval=FALSE}
PharmacoGx::downloadPSet('CCLE')
PharmacoGx::downloadPSet('GDSC')
```

Specify file paths, load data sets, and find common cells and drugs between CCLE and GDSC.
```{r}
...
p2 <- tdat2 %>%
  ggplot(aes(x=concentrations,y=value)) +
    ...
    #geom_vline(data=as.tibble(fit2),aes(xintercept=EC50),lty=2,lwd=.5,alpha=.5) +
    #geom_hline(data=as.tibble(fit1),aes(yintercept=E_inf),lty=2,lwd=.5,alpha=.5) + 
