...
output_dir = file.path('~/Downloads', 'results', paste(paste(datasets,collapse='_'),'fit_within_study',sep='_'))
...
for (nn in names(intersection.fitted.list)) {
  intersection.fitted.list[[nn]] %>% 
    ...
    filter((cell_type == 'sensitive' & posterior_probability_sensitive >= .5 & drug_type == 'targeted') | 
           ...
           drug_type == 'broad') %>% 
    select(drug, cell, experiment, realized_value, drug_type, 
           posterior_probability_targeted, cell_type,
