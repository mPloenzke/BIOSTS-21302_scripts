...
opt$log_dir <- file.path('~/Downloads', 'Pharmacogenomics', 'results',paste(paste(opt$datasets,collapse='_'),'fit_p60',sep='_'))
...
```{r}
for (dataset in opt$datasets) {load(file.path(base_dir, 'PSets',paste(dataset,'RData',sep='.')))}
```
...
write_csv(posterior, file=file.path(opt$log_dir,'posteriors.csv'))
```
