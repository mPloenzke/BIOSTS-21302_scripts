...
```{r}
  ...
  base_dir <- file.path('~/Downloads/Pharmacogenomics')
  #opt$datasets <- c('CCLE','GDSC1000', 'gCSI', 'FIMM', 'CTRPv2') # CCLE GDSC GDSC1000 gCSI FIMM CTRPv2
  opt$biomarkers_file <- file.path(base_dir, 'PSets/gene_drug_asociations.xlsx')
  ...
  opt$log_dir <- file.path(base_dir, 'results',paste(paste(opt$datasets,collapse='_'),'fit_p60_2',sep='_'))
...
```{r}
  for (dataset in opt$datasets) {
    if (opt$run_on_cluster) {
      ...
      load(file.path(base_dir, 'PSets',paste(dataset,'RData',sep='.')))
    }
  }
