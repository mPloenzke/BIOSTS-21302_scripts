...
```{r}
  ...
  opt$sensitivity_measure <- 'aac'
  opt$log_dir <- paste(paste(opt$datasets,collapse='_'),'fit_overlap',sep='_')
...
```{r}
  ...
  if (opt$run_on_cluster) {
    base_dir <- 'PSets/prism'
  } else {
    base_dir <- '~/Desktop/Concordance/PSets/prism'
  }
```

Load datasets.
```{r}
aucs_published <- read_excel(file.path(base_dir,"43018_2019_18_MOESM2_ESM.xlsx"), 
    sheet = "11-Reprocessed AUCs", 
    skip = 2) %>%
  as_tibble() %>%
  select(Drug.Name, ccle, REP_auc_overlap, CTD2_auc_overlap, GDSC_auc_overlap) %>%
  gather('experiment','value',-Drug.Name,-ccle) %>%
  separate(experiment,into=c('experiment','measure','v3'),sep='_') %>%
  select(-v3) %>%
  mutate(value = 1-value) %>%
  rename(drug = Drug.Name,
         cell=ccle) %>%
  mutate(experiment=case_when(experiment=='REP' ~ 'PRISM',
                              experiment=='CTD2' ~ 'CTRP',
                              TRUE ~ experiment),
         drug=tolower(drug)) %>%
  mutate(measure = 'aac') %>%
  group_by(drug, cell, measure, experiment) %>%
  arrange(value) %>%
  mutate(cell2 = paste(cell,row_number(),sep='_')) %>%
  ungroup() %>%
  mutate(cell = cell2) %>% 
  select(drug, cell, value, experiment, measure) 
prism <- aucs_published %>% filter(experiment=='PRISM')
ctrp <- aucs_published %>% filter(experiment=='CTRP')
gdsc <- aucs_published %>% filter(experiment=='GDSC')
```
